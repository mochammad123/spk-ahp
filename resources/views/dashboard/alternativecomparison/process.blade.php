                ...
                <div class="card-body">
                    <div class="table-responsive">
                            <table class="table table-bordered datatab" id="dataTable" width="100%" cellspacing="0">
                                <thead>
                                    <input type="text" hidden style="font-size: 14px" class="form-control" id="criteria1" name="criteria1" required autofocus value="{{ $n = $alternatives->count() }}">


                                    @foreach ($criterias as $criteria)
                                    <input type="text" style="font-size: 14px" class="form-control" id="criteria" name="criteria" hidden required autofocus value="{{ $pilihan[] = $criteria->criteria }}">
                                    <input type="text" hidden style="font-size: 14px" class="form-control" id="criteria" name="criteria" required autofocus value="{{ $input = $criteria->id }}">
                                    @endforeach
                                    <tr>
                                        <th>Alternatif</th>
                                        @foreach ($alternatives as $alternative)
                                            @foreach ($columns as $column)
                                                @if ($alternative->id == $column)
                                                <th>{{ $alternative->alternative }}</th>
                                                @endif
                                            @endforeach
                                        @endforeach
                                    </tr>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered datatab" id="dataTable" width="100%" cellspacing="0">
                            ...
                            <tbody> 
                                ...
                                @foreach ($supportRows as $alternative1 => $columns)
                                <tr>
                                    @foreach ($alternatives as $alternative)
                                    @if ($alternative->id == $alternative1)
                                    <td>{{ $alternative->alternative }}</td>
                                    @endif
                                    @endforeach  
                                    @foreach ($columns as $alternative => $grade)                                        
                            ...
                            <table class="table table-bordered datatab" id="dataTable" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @endif
                                        @endforeach
                                        @foreach ($rank as $alternative2 => $grade)                                        
                                        <td>{{ number_format($grade, 2) }}</td>
                                        @endforeach
                                        <td>{{ $no++ }}</td>
                                    </tr>
        ...
        <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        ...
                        <form method="POST" action="/dashboard/priorityalternative" enctype="multipart/form-data">
                            @csrf
                        @foreach ($cl as $item)
                            <input type="text" style="font-size: 14px" class="form-control" id="criteria_id" name="criteria_id[]" hidden required value="{{ $item }}">
                        @endforeach
                        @foreach ($scl as $item)
                            <input type="text" style="font-size: 14px" class="form-control" id="alternative_id" name="alternative_id[]" hidden required value="{{ $item }}">
                        @endforeach
                        @foreach ($sp as $item)
